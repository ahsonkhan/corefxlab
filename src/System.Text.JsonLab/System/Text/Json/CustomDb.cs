...
namespace System.Text.JsonLab
{
    internal struct CustomDb
    {
        ...
        public void Resize()
        {
            Memory = Memory.Slice(0, Index);
        }

        public Span<byte> Slice(int startIndex) => Memory.Span.Slice(startIndex);

        public Span<byte> Slice(int startIndex, int length) => Memory.Span.Slice(startIndex, length);
        ...
        public void Append(JsonType jsonType, int startLocation, int LengthOrNumberOfRows = DbRow.UnknownSize)
        {
            ...
            if (Index >= Memory.Length - DbRow.Size)
                Enlarge();
            var dbRow = new DbRow(jsonType, startLocation, LengthOrNumberOfRows);
            MemoryMarshal.Write(Memory.Span.Slice(Index), ref dbRow);
            Index += DbRow.Size;
        }
        ...
        {
            int size = Memory.Length * 2;
            byte[] newArray = _pool.Rent(size);
            Memory<byte> newDbSpace = newArray;
            Memory.Span.Slice(0, Index).CopyTo(newDbSpace.Span);
            Memory = newDbSpace;
            _pool.Return(_rentedBuffer);
        ...
        {
            Debug.Assert(index >= 0 && index <= Memory.Length - DbRow.Size);
            Debug.Assert(length >= 0);
            MemoryMarshal.Write(Memory.Span.Slice(index + 4), ref length);
        }
        ...
        public void SetNumberOfRows(int index, int numberOfRows)
        {
            ...
            byte previous = (byte)(Memory.Span[index + 11] >> 4);
            int value = (previous << 28) | numberOfRows;
            MemoryMarshal.Write(Memory.Span.Slice(index + 8), ref value);
        }
        ...
        public void SetHasChildren(int index)
        {
            ...
            index += 11;
            byte previous = Memory.Span[index];
            Memory.Span[index] = (byte)(1 << 7 | previous);
        }
        ...
        {
            for (int i = 0; i < Memory.Length; i += DbRow.Size)
            {
                DbRow row = MemoryMarshal.Read<DbRow>(Memory.Span.Slice(i));
        ...
        public DbRow Get() => MemoryMarshal.Read<DbRow>(Memory.Span);

        public DbRow Get(int index)
        {
            Debug.Assert(index >= 0 && index <= Memory.Length - DbRow.Size);
            return MemoryMarshal.Read<DbRow>(Memory.Span.Slice(index));
        }

        public int GetLocation() => MemoryMarshal.Read<int>(Memory.Span);

        public int GetLocation(int index)
        {
            Debug.Assert(index >= 0 && index <= Memory.Length - DbRow.Size);
            return MemoryMarshal.Read<int>(Memory.Span.Slice(index));
        }

        public int GetSizeOrLength(int index)
        {
            Debug.Assert(index >= 0 && index <= Memory.Length - DbRow.Size);
            return MemoryMarshal.Read<int>(Memory.Span.Slice(index + 4));
        }

        public JsonTokenType GetJsonType(int index = 0)
        {
            Debug.Assert(index >= 0 && index <= Memory.Length - DbRow.Size);
            int union = MemoryMarshal.Read<int>(Memory.Span.Slice(index + 8));
        ...
        public string PrintDatabase()
        {
            ...
            for (int i = 0; i < Memory.Length; i += DbRow.Size)
            {
                DbRow record = MemoryMarshal.Read<DbRow>(Memory.Span.Slice(i));
